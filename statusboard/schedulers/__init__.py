...
from statusboard.schedulers.models import (PeriodicTask, PeriodicTasks,
                             CrontabSchedule, IntervalSchedule, metadata)
...
class ModelEntry(ScheduleEntry):
    ...
    def next(self):
        ...
        return self.__class__(self.session, self.model)
    ...
    @classmethod
    def from_entry(cls, session, name, skip_fields=("relative", "options"), **entry):
        fields = dict(entry)
        ...
        obj = session.query(PeriodicTask).filter_by(name=name).first()
        if not obj:
            obj = PeriodicTask(name=name)
            session.add(obj)
        for key, value in fields.iteritems():
            setattr(obj, key, value)
        return cls(session, obj)
...
class DatabaseScheduler(Scheduler):
    ...
    def __init__(self, *args, **kwargs):
        self.db_uri = kwargs.get("schedule_filename")
        self.engine = None
        self.session = None
    ...
    def all_as_schedule(self):
        ...
        s = {}
        for model in self.session.query(self.Model).filter_by(enabled=True):
            try:
                s[model.name] = self.Entry(self.session, model)
            except ValueError:
                pass
        return s
    ...
    def schedule_changed(self):
        if self._last_timestamp is not None:
            ts = self.Changes.last_change(self.session)
    ...
    def update_from_dict(self, dict_):
        ...
        for name, entry in dict_.items():
            try:
                s[name] = self.Entry.from_entry(self.session, name, **entry)
